...
instantaneously.
"""
from socket import socket, SOCK_STREAM, AF_INET, getfqdn
...
class Games:
    ...
    :var all_games: A list of all games in the lobby.
    :type all_games: list of Game objects."""
    def __init__(self,port_number):
        self.all_games = []
        self.sock = socket(AF_INET, SOCK_STREAM)
        server_addr = (getfqdn(), port_number)
                    ...
                    for game in self.all_games:
                        if game.room_code() == msg["check_room_code"]:
                            ...
                            data = {"exists": True, "room_code": game.room_code(),
                                    "num_of_players": game.num_of_players(), "game_id": game.game_id()}
                            data = json.dumps(data)
                            connection.sendall(data.encode())
                            break
                    if not exists:
